<script setup>
import AppLayout from './AppLayout.vue'

defineProps({
  movies: {
    type: Array,
    required: true
  },
  loading: {
    type: Boolean,
    required: true
  }
})
</script>
<template>
  <AppLayout class="mt-20">
    <h2 class="text-[#767E94] font-semibold text-[32px]">
      All <span class="text-base">({{ movies ? movies.length : '0' }})</span>
    </h2>
    <div v-if="!loading" class="grid grid-cols-2 min-[500px]:grid-cols-3 sm:grid-cols-4 gap-6 mt-6">
      <div
        v-for="movie in movies"
        :key="movie?.show?.id"
        style="background: rgb(77 80 91 / 20%)"
        class="rounded-xl backdrop-blur-[40px] px-2 pt-2 pb-4 relative cursor-pointer"
      >
        <div
          style="background: rgba(0, 0, 0, 0.65)"
          class="absolute top-[18px] left-4 backdrop-blur-[40px] rounded-lg p-2 flex gap-1 items-center"
        >
          <svg
            width="16"
            height="16"
            viewBox="0 0 16 16"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
            <path
              d="M9.15327 2.33999L10.3266 4.68666C10.4866 5.01333 10.9133 5.32666 11.2733 5.38666L13.3999 5.73999C14.7599 5.96666 15.0799 6.95333 14.0999 7.92666L12.4466 9.57999C12.1666 9.85999 12.0133 10.4 12.0999 10.7867L12.5733 12.8333C12.9466 14.4533 12.0866 15.08 10.6533 14.2333L8.65994 13.0533C8.29994 12.84 7.70661 12.84 7.33994 13.0533L5.34661 14.2333C3.91994 15.08 3.05327 14.4467 3.42661 12.8333L3.89994 10.7867C3.98661 10.4 3.83327 9.85999 3.55327 9.57999L1.89994 7.92666C0.926606 6.95333 1.23994 5.96666 2.59994 5.73999L4.72661 5.38666C5.07994 5.32666 5.50661 5.01333 5.66661 4.68666L6.83994 2.33999C7.47994 1.06666 8.51994 1.06666 9.15327 2.33999Z"
              stroke="#FFAD49"
              stroke-width="1.5"
              stroke-linecap="round"
              stroke-linejoin="round"
            />
          </svg>
          <p class="text-[#FFAD49] text-base">
            {{ movie?.show?.rating?.average?.toFixed(1) ?? '??' }}
          </p>
        </div>
        <img
          :src="movie?.show?.image?.original"
          alt="banner image"
          class="w-full aspect-[1/1.7] xl:aspect-[1/1.6] rounded-lg"
        />
        <h3 class="mt-4 text-[#EBEEF5] text-base font-semibold">{{ movie?.show?.name }}</h3>
      </div>
    </div>
    <div v-else class="grid grid-cols-2 min-[500px]:grid-cols-3 sm:grid-cols-4 gap-6 mt-6">
      <div
        v-for="n in 8"
        :key="n"
        style="background: rgb(77 80 91 / 20%)"
        class="w-full aspect-[1/8] rounded-xl backdrop-blur-[40px] animate-pulse px-2 pt-2 pb-4"
      ></div>
    </div>
  </AppLayout>
</template>
